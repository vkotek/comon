...
function comon_active_posts() {
    ...
    while ($the_query -> have_posts()) : $the_query -> the_post();
        if ( ideablog_data_filter() ) :
            if ( comon_expiry() ) : ?>
                <li><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a> | <?php printf(_n('<b>%s</b> day left','<b>%s</b> days left', comon_expiry('days')), number_format_i18n(comon_expiry('days'))); ?></li>
            <?php
            endif;
        endif;
    endwhile;
    ...
    while ($the_query -> have_posts()) : $the_query -> the_post();
        if ( ideablog_data_filter() ) :
            if ( !comon_expiry() ) : ?>
			    ...
			    <a href="<?php the_permalink(); ?>"><?php the_title(); ?></a> | <?php printf(_n('<b>%s</b> day left','<b>%s</b> days left', comon_expiry('days')), number_format_i18n(comon_expiry('days'))); ?></li>
            <?php
            endif;
        endif;
    endwhile;
